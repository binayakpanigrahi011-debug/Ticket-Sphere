...
const HeroCarousel = ({ movies }) => {
  ...
  return (
    <div className="carousel-container animate-fade-in" style={{ backgroundColor: 'var(--bg-color)' }}>
        ...
        >
          ...
          <div className="carousel-content" style={{ position: 'relative', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 5rem', maxWidth: '800px', zIndex: 10 }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
              <span style={{ padding: '0.4rem 1rem', background: 'rgba(255, 255, 255, 0.2)', backdropFilter: 'blur(4px)', color: '#fff', borderRadius: '4px', fontSize: '1rem', fontWeight: 'bold' }}>{movie.genre}</span>
              <span style={{ padding: '0.4rem 1rem', background: 'rgba(255, 255, 255, 0.2)', backdropFilter: 'blur(4px)', color: '#fff', borderRadius: '4px', fontSize: '1rem', fontWeight: 'bold' }}>{movie.duration}</span>
            </div>
        ...
        <div style={{ 
          ...
          {movies.map((movie) => (
            <Link to={`/movie/${movie._id}`} key={movie._id} className="card" style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ padding: '1.2rem', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', background: 'var(--card-bg)' }}>
                <div>
